...
def test_product_communication_package_contract():
    ...
    assert re.search(r"^version: 1\.3\.6$", skill, re.MULTILINE)
...
def test_product_communication_anchors_updates_to_the_active_project_and_explains_new_issues():
    skill = (SKILL_DIR / "SKILL.md").read_text(encoding="utf-8")
    evaluation = yaml.safe_load((SKILL_DIR / "EVAL.yaml").read_text(encoding="utf-8"))
    fixture = (SKILL_DIR / evaluation["parameters"]["fixture"]).read_text(
        encoding="utf-8"
    ...
    expectations = "\n".join(evaluation["expectations"])
    contract = "\n".join((skill, fixture, expectations)).lower()
    ...
    assert "do not send a bare list of internal finding names" in contract
    assert "what this changes for the active project" in contract
    assert "define or replace every unavoidable internal term" in contract
