...
def test_issue_monitor_uses_official_kanban_for_live_workers() -> None:
    ...
    assert "Gateway-owned continuation" in content
    assert "Kanban terminal state and heartbeats" in content
    assert "official Kanban" in expectations
    assert "heartbeat" in expectations


def test_scheduled_issue_monitor_dispatches_one_official_kanban_pass() -> None:
    content = skill_text("issue-monitor")
    expectations = eval_expectations("issue-monitor")
    ...
    assert "official Hermes cron job" in fixture
...
def test_completion_hooks_do_not_create_a_second_issue_monitor_controller() -> None:
    subagent = skill_text("subagent-driven-development")
    reliability = skill_text("delegation-reliability")
    ...
    assert "completion-triggered scheduling wake" not in issue_monitor
    assert "Gateway-owned continuation" in issue_monitor
    assert "Do not add a lifecycle plugin" in issue_monitor
    for name in ("subagent-driven-development", "delegation-reliability"):
...
def test_eval_fixtures_cover_profile_harvested_boundaries() -> None:
    required = {
        ...
        "issue-monitor": ("official Kanban", "dispatch --max 1 --json"),
    }
