...
The publishing report and every non-silent user update open with a natural purpose sentence without a `Purpose:` label, then use `Executive summary:`, `Human action needed:`, and `Detailed information:` and lead with the tester or release outcome. The human-action field is exactly `None` unless a human owns the decision/task; populated actions name the actor, imperative action, timing, result unblocked, and why automation cannot perform it safely. Autonomous monitor/team work remains in the executive summary. Technical account, artifact, version, policy, and evidence fields remain under detailed information.
