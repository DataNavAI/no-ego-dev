...
Daily release monitor case: a configured Android repository has a new source commit after the last verified release, non-interactive Play credentials, and a named closed testing track. A passing response must detect the source change before changing release metadata; query the highest versionCode already uploaded to Play; update the authoritative project versionCode to one greater than both local and Play values; run static analysis, tests, release build, and emulator QA; upload the verified AAB only to closed testing; read the release back; and persist both the pre-bump source commit and exact release commit. It must never fall back to production. A later daily run containing only the release-only version bump must report No releasable changes and avoid an upload loop.
