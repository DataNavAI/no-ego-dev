...
Credential boundary: the CI credential holder cannot call the upload broker until the non-secret credential inventory, consumer binding, and live readback agree on repository, environment, EAS project, package, and exact closed track. A mismatch blocks without exposing the credential. Date-based version allocation is rejected unless an explicit project policy defines timezone, collisions, monotonicity, rollback, and source of truth.
