...
When a target has a materially different same-name package, inspect every complete-package delta regardless of version or baseline state and assign each behavior/support-file delta one ledger disposition: `adopted`, `scoped`, `superseded`, `product-local`, `unsafe`, or `unresolved`. Explicit standardization authority cannot bypass this ledger, safety, canonical publication, or immutable-source gates. Reusable target drift must be re-harvested into a new validated, reviewed, merged canonical generation before overwrite; `product-local` adaptations must be declared and hash-verified; `unsafe` or `unresolved` deltas block only that package/profile with state unadvanced. A backup is rollback protection, never permission to discard unharvested behavior.
