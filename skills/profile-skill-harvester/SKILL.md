...
10. **No live-only harvest completion.** Every new or updated reusable skill selected during a harvest—including an out-of-band global/default installed variant, an enrolled sibling-profile variant, or a correction authored during reconciliation—must be added to `/Users/moonk/no-ego-dev` as a complete package and merged into the remote default branch before any sibling rollout. Canonical publication is a prerequisite to rollout. A skill absent from the canonical repository must not be deployed directly; import it with eval coverage or classify it as blocked/rejected. Live profile rollout is not publication. The executable inventory baseline covers exactly the five enrolled NED profile roots; global/default observations remain separate durable continuation inputs and must never be represented as enrolled-profile digest advancement.
...
1. Parse every changed `SKILL.md` frontmatter and every changed scenario file matching `EVAL*.yaml`.
   - Reject symlinked `SKILL.md`, `EVAL*.yaml`, fixtures, templates, and support files. Hash only self-contained regular package files; never silently omit a symlink from the package digest.
...
2. Ensure every fixture and template referenced by every package-local `EVAL*.yaml` is a non-empty regular package-relative file; reject absolute paths and lexical `..` before resolution.
