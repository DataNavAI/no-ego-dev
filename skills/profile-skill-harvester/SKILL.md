...
An unfinished automation PR is a durable continuation, not a report-only blocker. Apply **resume-before-inventory**: reconcile any existing PR, local-ahead worktree, continuation marker, failed check, and exact-SHA review before scanning for new work. Diagnose and repair failures within the harvester's existing authority, generate required evidence-only commits, use bounded retries only for proven external transients, merge with an exact head guard, verify applicable default-branch CI, complete authorized rollout, and release the exact lock on every terminal path. Follow [`references/self-unblocking-publication.md`](references/self-unblocking-publication.md). Routine CI/evidence failures must not be handed back to the user when the scheduled job can safely resolve them itself.
