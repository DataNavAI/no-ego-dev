...
## Scenario: Per-project worker watchdog startup and reconciliation

A new project uses a canonical GitHub repository and issue tracker and already has an older scheduled monitor and worker-pool rule. A passing startup derives stable repository/tracker identity, searches technical scope, and creates or updates exactly one per-project Hermes cron watchdog with a friendly project outcome name. It reconciles the scheduled monitor and worker-pool to one dispatch authority rather than leaving duplicate controllers. Its self-contained prompt records repository/tracker coordinates, task states and dependency policy, profile routing, project lock/state paths, cadence, and evidence rules; a fresh cron run never creates cron jobs or asks for chat context, preserving no recursive cron creation.

Setup lists and reads back the exact job, performs one manual reconciliation, and verifies the receipt/output before project startup is called complete. A pre-existing user pause remains paused. Explicit project pause, archive, or completion must pause or retire the watchdog.

On each run, the project-scoped lock and atomic claim make overlapping or retried ticks dispatch at most once. Worker activity requires a durable claim plus live lease/runtime evidence; pid, branch, PR, stale claim, and self-report alone are insufficient. Given runnable tasks and `active_worker_count == 0`, the run atomically claims the highest-priority dependency-safe task and launches exactly one linked focused worker. Given `active_worker_count >= 1`, it launches none. Given `eligible_count == 0`, it launches none. Stale claims are recovered only through bounded verified lease rules, and an attempt releases only its own lock.
