...
## Scenario: Official per-project cron + Kanban reconciliation

A new project uses a canonical GitHub repository/tracker and already has duplicate scheduled monitors plus a worker-pool launcher. Project-manager verifies the canonical checkout/remote, active profile/home, stable board, exact `hermes` executable, and effective non-boolean integer `kanban.max_in_progress=1` (active-profile runtime, all boards). It renders a project-specific bounded safe no-agent script into the active profile's `scripts/` directory outside the repository, reads back exact bytes/hash, and manually runs that installed file with `--dry-run` before cron activation.

The fixture `cronjob-list-real-no-agent.json` was captured from an isolated official `cronjob(action="create")` then `cronjob(action="list")`. Reconciliation parses real `job_id`, `name`, `prompt_preview`, `script`, `no_agent`, `workdir`, `enabled`, `state`, schedule, and metadata fields. It matches exact safe filename + canonical workdir + friendly name + optional bound ID, rejects duplicates/partial collisions/unknown pause encodings, preserves pause, converges official duplicates, and requires a fresh exact-one re-list. A new paused project stages valid create → canonical response `job_id` → immediate pause → fresh exact-one paused readback. The job is `script=<relative safe filename>`, `no_agent=True`, with no prompt/skill/toolset.

Subprocess E2E executes the rendered standalone script with a fake `hermes` executable that records argv and serves JSON. Its isolated production scheduler environment supplies exact `HERMES_HOME` but no profile-name env; absence is allowed, while present mismatches fail before subprocess. It proves verified no-op has empty stdout; any running claim no-ops until official gateway stale reclaim returns ready; ready plus zero running invokes exactly one fixed `dispatch --max 1 --json`; dry run records only three read commands and a zero-mutation receipt; and hostile IDs, malformed/conflicting/unknown fields, duplicate IDs, bad dispatch output, path/URL attacks, or identity drift fail closed. Complete task/stats/dispatch schemas and non-boolean nonnegative mutually consistent running evidence are mandatory. The script never loops or invokes cron/chat/delegation, and it does not claim `show`/`runs` exposes unavailable heartbeat fields.

Lifecycle fixtures prove pause/remove requires a current exact binding and a fresh paused/absent readback. Older issue-monitor/worker-pool routes converge to this one board and no-agent job; unrelated issue workflow protocols remain worker-owned.
