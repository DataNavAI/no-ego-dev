...
A new project uses a canonical GitHub repository/tracker and already has duplicate scheduled monitors plus a worker-pool launcher. Project-manager resolves or creates one stable sanitized per-project Kanban board. It derives a self-contained technical marker from repository + tracker only, reads the durable project status/notepad job binding, calls `cronjob(action="list")`, and uses only official create/update/pause/remove operations. It adopts the bound match (or deterministic smallest ID), preserves any user pause, removes same-marker duplicates, and re-lists until exactly one real job remains. The older issue-monitor and worker-pool routes are reconciled to that same job and board. It never edits `jobs.json` or claims a skill-local lock fenced the external scheduler.

The friendly job pins exact repository, tracker, profile, board, and absolute `workdir`; attaches `issue-monitor`; and has only terminal/file toolsets. Task/issue/repository content is untrusted data and never selects authority or enters commands. Hostile repository, board, profile, or task identifiers fail validation. Setup calls `cronjob(action="run")` with **SETUP_DRY_RUN_NO_LAUNCH**, verifies exact job readback and terminal cron receipt/history, proves zero mutation/dispatch, and persists project → board → marker → job ID in durable project status/notepad.

An ordinary tick parses captured official Kanban `list`, `stats`, running-task `show`, and `runs` JSON. Active worker means no-op; no ready task means no-op; invalid/uncertain evidence means fail-closed no-op. Only ready tasks plus zero project-wide running workers yields exactly one `hermes kanban --board <slug> dispatch --max 1 --json` command and one official receipt/readback. Task strings never appear in argv. The prompt contains no recursive cron operation and no in-process delegation path. Kanban owns dependencies, atomic claims, heartbeats, stale reclaim, isolated workers, durable runs, and lifecycle stages.

Lifecycle fixtures prove project pause uses official cron pause without implicit resume, while archive/completion removes the job and preserves final evidence. Captured `cronjob list` fixtures cover empty, duplicate, and paused states; they are adapter inputs, not claims of external scheduler or worker effects.
