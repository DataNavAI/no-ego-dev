...
Before review, require a machine-readable **review-readiness receipt** bound to the exact candidate and base. It proves clean scope plus green static analysis, focused tests, canonical full tests, build, secret scan, provider checks, and controller self-audit. Incomplete candidates return to implementation without consuming a review round. An atomic candidate/review index suppresses active or finalized same-SHA duplicates, allows one missing-evidence-only recovery from `INCOMPLETE`, and records review runtime, fresh-token use when available, attempt counts, suppressions, candidate generations, and Round-3 frequency.
