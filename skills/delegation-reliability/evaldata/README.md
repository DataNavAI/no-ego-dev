...
- `/agents` is empty or misleading while a gateway child may still be active; the controller must answer a queued ledger-status request, distinguish confirmed/unknown/terminal state, and offer hook-ledger or Kanban commands without claiming another process can inspect the gateway registry;
