...
8. **Route by verdict, not by nominal workflow position.** A recovered `REQUEST_CHANGES` composite verdict triggers a narrow remediation worker and a fresh exact-head composite rereview. It must not advance to merge or dependent work. A recovered `APPROVED` verdict still requires digest/candidate verification before the merge-only gate.
