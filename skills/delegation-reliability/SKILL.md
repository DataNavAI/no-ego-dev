...
- `/agents` (alias `/tasks`) is not an authoritative delegated-child view on gateway messaging surfaces. Do not use its output or absence as lifecycle evidence.
...
**Hook-only mode (preferred when the user rejects queue duplication):** keep GitHub/Linear canonical and rely on each async child completion delivery to re-enter the parent session. A top-level batch produces independent child handles and deliveries; separate single-task calls remain useful for explicit per-call ownership or retry boundaries. The parent verifies the durable artifact and verdict, reconciles live tracker state, and immediately dispatches the next dependency-safe worker. `subagent_stop` is observer-only and cannot itself prove success or launch a child through its ignored return value. This mode is process-local and must not be described as restart-durable.
...
8. **Close supervision.** Once completion, interruption, or timeout is confirmed, close the active ledger row; recovered artifacts affect the task verdict, not the run-state classification, and liveness checks should become silent.
...
- Do not re-dispatch because `/agents` is empty; confirm interruption, absence, or staleness from lifecycle evidence and target write safety.
...
- [ ] Top-level batch children are tracked and reconciled independently; separate calls are used only when explicit per-call ownership or retry boundaries are required.
...
- [ ] The controller/hook/Kanban ledger states the evidence source for every active or unknown row; `/agents` was not used as proof.
