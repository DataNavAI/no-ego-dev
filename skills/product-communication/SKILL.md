...
license: MIT
created_by: NoEgoDev
adapted_from: Hermes Agent profile variants
metadata:
  hermes:
...
## Active-Project Anchoring and Contextual Issues

Every user-facing update must be understandable as part of the **active project and requested outcome**, not merely accurate about internal work.

1. Open with the active project, requested outcome, and current delivery state. The first sentence must not be reusable unchanged for an unrelated project.
2. Before introducing a newly discovered issue, explain:
   - **Discovery:** why it is being mentioned now.
   - **Relationship:** how it affects the requested project or outcome.
   - **Consequence:** what a user may experience or what delivery risk changed.
   - **Response:** what the team is doing and the next product checkpoint.
   - **Action:** what the user must do, or exact `Human action needed: None`.
3. Translate internal finding names into grouped product consequences such as gameplay, reliability, visual clarity, keyboard accessibility, or release confidence. Do not lead with raw labels, finding counts, worker orchestration, regression mechanics, or correction-set terminology.
4. If the relationship is not verified, do not invent it. Say the issue was found during work on the active project, its effect on the requested outcome is still being verified, and when that relationship will be reported.
5. Treat a bare internal finding list as a communication failure even when every label is technically correct.

Progress updates follow the same rule: report the current product state and next product checkpoint, not which worker is adding tests or which internal correction set is being processed.

See [`references/project-context-and-issue-translation.md`](references/project-context-and-issue-translation.md) for the reported failure pattern, decision procedure, and reusable rewrites.
...
- Do not ask the user to restart, configure, or run a command without explaining which product outcome or delivery stage remains blocked, why automation cannot safely perform it, and what happens after the action.
- Distinguish the **immediate boundary** from the **full completion path**. If the requested action clears only the first preflight gate, say so explicitly and name the remaining verified gates; never imply that one restart, approval, or access grant will publish, deploy, or complete the project when merge conflicts, evidence, review, CI, rollout, or adoption checks still remain.
- Explain restart boundaries in product terms: which running controller predates which configuration, what runtime behavior therefore cannot be proved, why the owning request cannot safely restart itself, and which checkpoint the next fresh request will resume from. Keep PIDs and timestamps secondary unless they help verify the claim.
- If only part of the project is blocked, state what will continue independently.

## Link and Evidence Rules

### Source authority, authentication, and queued-state corrections

When correcting a status that mixes planning, authentication, and scheduler evidence, state each boundary in a complete plain-language sentence:

- Label an internal planning transcript as **internal planning**, not stakeholder approval. Name the smallest remaining authority check: confirmation of the public product promise from the named accountable owner.
- Distinguish a browser sign-in, an OAuth client, and configured scopes from a verified refreshable token. Those setup facts do not prove the service can authenticate unattended.
- State that end-to-end operation remains unverified until a real operation and authoritative readback succeed.
- A successful scheduler reconciliation proves only that scheduling logic ran. If no worker is running and the next task is queued, say **queued—not started**, never active or restored.
- If the recipient need not act, keep owner-confirmation and delivery verification in `Executive summary` and write `Human action needed: None` exactly.

Do not compress these distinctions into fragments joined by `but`, slashes, or parenthetical shorthand; source authority and authentication state must remain understandable after one read.
...
For verified-running versus queued work, unchanged-worker silence, stale approval and documentation reconciliation, source-authority labels, common-language security risks, authentication-state claims, and reader-clarity corrections, follow [`references/progress-and-evidence-language.md`](references/progress-and-evidence-language.md).
...
**Executive summary:** <who is affected, what they can now do or what risk changed, any remaining limitation, and the autonomous/team next product step>
...
See [`references/decision-ready-message-examples.md`](references/decision-ready-message-examples.md) for additional rewrites and a pre-send checklist. For controller/gateway restart requests, first-gate semantics, and messages that must not imply restart alone completes publication, see [`references/restart-boundary-vs-completion.md`](references/restart-boundary-vs-completion.md).
...
- [ ] The opening sentence names the active project, requested outcome, and current product state; it states the product's before→after change or present decision/blocker naturally and does not use a `Purpose:` prefix
- [ ] Every newly introduced issue explains why it appeared now, its verified relationship to the active project (or explicit uncertainty), user consequence, current response, and next product checkpoint
- [ ] Internal finding labels, worker orchestration, and remediation mechanics are translated into product consequences or moved out of the user-facing lead
...
- [ ] A requested external action is framed as the first boundary or the full unblock truthfully; if later review, evidence, CI, merge, rollout, or adoption gates remain, they are named and the message does not imply the action alone completes publication/deployment
