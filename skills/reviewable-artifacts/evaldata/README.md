...
Mode distinction check: a separate documentation change in this project is intended to land directly in the base branch. That PR must be classified `MERGEABLE`, must not receive review-only branch/title/body/label markers, and must follow the normal PR/CI/merge lifecycle. The temporary AtlasBoard design/spec review remains `REVIEW_ONLY` and follows close-without-merge cleanup.
