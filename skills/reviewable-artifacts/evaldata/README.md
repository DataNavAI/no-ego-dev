...
4. **Wrong thread identity:** a requested thread ID belongs to another PR. Show that the read-only inspect command requires exact repository, PR, thread ID, and expected head and therefore fails closed without exposing a mutation path.
