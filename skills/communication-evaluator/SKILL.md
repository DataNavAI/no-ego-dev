...
1. **Which active project and requested outcome does this concern?**
2. **What changed, failed, completed, or is waiting?**
3. **Who is affected?**
4. **Why it matters to them or to the product?**
5. **What a person must do, if anything?**
6. **What happens next?**
...
8. **Active-project context missing:** introduces review findings, internal tasks, or new issues without naming the active project and requested outcome, why the matter appeared now, what it changes for that project, and the next product checkpoint. A bare list of internal finding names hard-fails even when technically accurate. If the relationship is not verified, the message must say so rather than inventing impact.
...
- **`APPROVED`** — score is at least 85, no hard-fail gate applies, and the one-read test answers all six questions accurately enough to act.
...
- Active project and requested outcome: <plain-language answer | Missing or unclear>
...
Use `scripts/score_evaluation.py` to total dimension scores and apply the 85-point, hard-fail, and one-read gates deterministically. `evaldata/cases.yaml` freezes exact A–E score/verdict oracles for regression testing; prose review still supplies the evidence-based dimension scores.

## Review Examples

### Opaque technical status

> The canary is paused because the OTel exporter exceeded cardinality limits. CI is green but the promotion job is paused pending schema remediation.
...
10. **Assuming an internal finding list is self-explanatory.** Require the active project, requested outcome, discovery source, user-visible consequence, and next checkpoint; define or replace every unavoidable internal term.
...
- [ ] All six one-read questions are answered or marked missing
