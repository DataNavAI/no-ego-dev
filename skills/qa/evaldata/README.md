...
MVP scope: the product plan defines exactly one primary CUJ and two necessary supporting CUJs. QA must create a traceability matrix mapping those journeys to acceptance criteria, automated end-to-end/integration coverage, focused rule/persistence/recovery tests, zero-context manual smoke cases, supported interfaces, and evidence. Missing, stale, failed, or blocked primary-CUJ evidence blocks release.
