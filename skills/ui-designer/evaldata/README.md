...
- The user asks to compare onboarding directions and explicitly does not want a verbal explanation. A passing `ui-designer` response must create runnable HTML/CSS prototypes or equivalent concrete visual mockups, run them, capture clean mobile/desktop screenshots, and publish a rendered slide-like `DESIGN_REVIEW.md` with each variant/screen/hotspot on a stable `UI-*`/`A*` anchor.
- The review must open with a visual contact sheet or thumbnail index. Each direction/screen should behave like one presentation slide: a dominant visual or desktop/mobile pair, a short title, at most three concise bullets or roughly 75 words, and one decision prompt. Detailed research, tokens, annotation legends, and implementation notes belong in a linked brief or appendix, not before or between the core visual slides.
- A verbose design essay, long executive summary, or screen-by-screen prose description does not pass, even if it eventually links images. If concrete viewable artifacts cannot be produced, the design decision remains `BLOCKED`.
- The index should embed screenshots, link runnable previews, compare CUJ fit/action count/hierarchy/responsiveness/accessibility/implementation cost, and recommend a direction. Because the PR exists only for design review, it must use a `review-only/*` branch, `[REVIEW ONLY — DO NOT MERGE]` draft title/body warning, available labels, and a cleanup owner/trigger so the user can comment beside the exact visual item without implying merge authorization.
- NED must read unresolved review threads, update canonical prototype source, regenerate screenshots, verify, reply with the addressing revision, and resolve only addressed/agreed comments. Disputed design decisions remain open; thread resolution, design approval, engineering handoff, and merge are separate states. After preserving the selected design at its canonical handoff, it must close the review PR without merge and verify cleanup of the branch, worktree, temporary previews/captures/copies/access, and scratch assets.
