...
- `reviewable-artifacts`: Markdown 문서와 시각 디자인 묶음을 렌더링된 GitHub 초안 PR로 제공하고, 안정적인 인라인 댓글 앵커, 에이전트가 읽을 수 있는 피드백, 검증된 수정본, 해결 가능한 리뷰 스레드를 관리합니다. 임시 검토 화면은 `[REVIEW ONLY — DO NOT MERGE]`로 명시하고, 승인된 작업과 검토 기록을 보존한 뒤 병합하지 않고 닫으며 임시 브랜치·워크트리·프리뷰를 정리합니다.
- `mvp-planning`: 새 MVP를 하나의 핵심 사용자 문제와 1~3개의 핵심 사용자 여정으로 축소하고, 가장 짧고 직관적인 UX를 설계하며, 불필요한 범위를 제거하고 CUJ 단위 자동·수동 QA 게이트를 정의합니다.
...
- `english-copywriter`: 웹사이트와 모바일 앱의 간결한 영어 UI 문구를 작성·검토하며, UI가 스스로 설명할 수 있는 문구는 제거하고 명확성·신뢰·접근성·복구에 필요한 문구만 남깁니다.
- `ui-designer`: 실행 가능한 시각 방향, 스크린샷, `DESIGN_REVIEW.md` 리뷰 갤러리를 만들어 사용자가 말로만 설명된 디자인을 승인하는 대신 정확한 변형·화면·핫스팟을 비교하고 댓글을 남길 수 있게 합니다.
