...
```bash
i=$(mktemp) && curl -fsSL https://raw.githubusercontent.com/DataNavAI/no-ego-dev/248ae5966871da5d2a13305b9e69c48f964e7913/scripts/install.sh -o "$i" && { echo "8877f5ac61eeb7cb7586f5d7e83f37c146567c715dadc8a9f67a815f856cb42c  $i" | sha256sum -c - 2>/dev/null || echo "8877f5ac61eeb7cb7586f5d7e83f37c146567c715dadc8a9f67a815f856cb42c  $i" | shasum -a 256 -c -; } && bash "$i"; s=$?; rm -f "$i"; (exit "$s")
```

The command downloads first and executes only bytes matching the displayed installer SHA-256, so a changed mutable response cannot run. Only the operating system's `bash`, `curl`, `tar`, and SHA-256 utility are used. The installer does **not** use `sudo`, `git`, or a system Node.js/npm. It installs a private Node.js 22.14.0 runtime and NED revision `8232991c95caf5dc3eabf291411aaf2ec21545d0` under `~/.local/share/ned`, verifies both archives with pinned SHA-256 values, installs lockfile-pinned dependencies, manages an exact PATH block in `~/.profile`, `~/.zprofile`, and `~/.bashrc`, and starts `ned create`.
